...
int main()
{
  ...
  asock_context_t *context = asock_context_create(0, loop, 0, options);

  // Generate the shared response
  const char body[] = "<html><body><h1>Hello, world!</h1></body></html>";

  http_context_t *http_context = (http_context_t *) asock_context_ext(0, context);
  http_context->response = (char *) malloc(128 + sizeof(body) - 1);
  http_context->length = snprintf(http_context->response, 128 + sizeof(body) - 1,
      "HTTP/1.1 200 OK\r\nContext-Length: %ld\r\n\r\n%s", sizeof(body) - 1,
      body);
